...
/**
 * Class AddCustomFieldConditionCommandHandler
 *
 * @package AmeliaBooking\Application\Commands\CustomField
 */
class AddCustomFieldConditionCommandHandler extends CommandHandler
{
    /**
     * @param AddCustomFieldConditionCommand $command
     *
     ...
     */
    public function handle(AddCustomFieldConditionCommand $command)
    {
        ...
        if (!$fieldCondition instanceof CustomFieldCondition) {
            ...
            $result->setMessage('Could not add custom field condition.');
        ...
        $id = $customFieldConditionRepository->add($fieldCondition);
        $fieldCondition->setId(new Id($id));

        $result->setResult(CommandResult::RESULT_SUCCESS);
        $result->setMessage('Successfully added new custom condition field.');
        $result->setData([
